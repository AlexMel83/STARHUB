...
class UserService {
  async registration(email, password, role, trx) {
    ...
    await tokenService.saveToken(user[0].id, tokens.refreshToken, tokens.expRfToken, trx);
    return {
      ...tokens,
      user: user,
    };
  ...
  async login(email, password, trx) {
    ...
    const tokens = tokenService.generateTokens({ ...userDto });
    await tokenService.saveToken(userDto.id, tokens.refreshToken, tokens.expRfToken, trx);
